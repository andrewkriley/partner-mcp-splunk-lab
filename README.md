...
An **OpenTelemetry Collector** runs as **`splunk-otel-gateway`** when you use Compose profile **`otel`**. It listens for **OTLP** on **localhost** (defaults **24317** gRPC, **24318** HTTP) and forwards traces, metrics, and logs to Splunk **HEC** using `SPLUNK_HEC_TOKEN` from `.env`. All OTLP telemetry is routed to the dedicated **`otel`** index (Splunk app **`otel_app/default/indexes.conf`**), sourcetype **`otlp_gateway`**. Verify with `index=otel`.
